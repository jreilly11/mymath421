...
```{r}
selectInput(inputId = "fighter_stance_input", 
            label = h2("Select Fighter Stance"),
            choices = ufc_fighter_stance, 
            selected = 'Orthodox')
```
...
```{r}
renderPlotly({
  p <- df %>% filter(fighter_stance==input$fighter_stance_input) %>% ggplot(aes(x=fighter_height_cm, y=fighter_weight_lbs, color = over_under_500)) + geom_point() + theme_dark() + ggtitle("Comparison of Fighter Height (cm),\nWeight (lbs), & Record") + labs(caption = "Based on 1994-2023 UFC Fighter Data") + theme(plot.caption = element_text(hjust=1)) + theme(plot.title = element_text(hjust=0.5)) + theme(plot.title = element_text(size=10))
...
```{r}
renderPlotly({
  p3 <- df %>% filter(fighter_stance==input$fighter_stance_input) %>% ggplot(aes(x=fighter_stance, fill=over_under_500)) + geom_bar() + ggtitle("Number of Fighters by Fighter Stance\n& Record")+ labs(caption = "Based on 1994-2023 UFC Fighter Data") + theme(plot.caption = element_text(hjust=1)) + theme(plot.title = element_text(hjust=0.5)) + theme(plot.title = element_text(size=10)) + ylab('# of Fighters') + scale_x_discrete(guide = guide_axis(n.dodge = 2))
