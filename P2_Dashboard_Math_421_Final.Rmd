...
a rich history of Mixed Martial Arts (MMA) combat. This data contains the characteristics of UFC fighters spanning from 1994 to 2023, including but not limited height, weight, fighting stance, etc.
...
```{r}
p3 <- ggplot(df, aes(x=fighter_stance, fill=over_under_500)) + geom_bar() + ggtitle("Number of Fighters by Fighter Stance\n& Record") + theme(plot.caption = element_text(hjust=1)) + theme(plot.title = element_text(hjust=0.5)) + theme(plot.title = element_text(size=10)) + ylab('# of Fighters') + labs(caption = "Based on 1994-2023 UFC Fighter Data")
...
```{r}
p4 <- ggplot(df, aes(x=age, y=total_fights, color = over_under_500)) + geom_point() + ggtitle("Comparison of Age,\nTotal Fights, & Record") + theme(plot.caption = element_text(hjust=1)) + theme(plot.title = element_text(hjust=0.5)) + theme(plot.title = element_text(size=10)) + labs(caption = "Based on 1994-2023 UFC Fighter Data")
