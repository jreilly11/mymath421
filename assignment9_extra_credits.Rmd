...
df2 <- read_csv('vaccination-data.csv')
```
...
# Had to also rename COUNTRY to Country in df2
df2 <- df2 %>% rename(Country = COUNTRY)

# Assuming Date_reported and DATE_UPDATED (variable in data source) refer to the same desired column
df2 <- df2 %>% rename(date = DATE_UPDATED)
```
...
```{r}
# Converting date in df to a date type (to be consistent with date in df2)
df$date <- as.Date(as.character(mdy(df$date)))

joined_df <- inner_join(df, df2, by = c('Country', 'date'))
```
