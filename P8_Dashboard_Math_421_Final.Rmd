...
```{r}
renderPlotly({
  p <- df %>% filter(fighter_dob > input$dob_input[1], fighter_dob < input$dob_input[2]) %>% ggplot(aes(x=fighter_height_cm, y=fighter_weight_lbs, color = over_under_500)) + geom_point() + theme_dark() + ggtitle("Comparison of Fighter Height (cm),\nWeight (lbs), & Record") + labs(caption = "Based on 1994-2023 UFC Fighter Data") + theme(plot.caption = element_text(hjust=1)) + theme(plot.title = element_text(hjust=0.5)) + theme(plot.title = element_text(size=10))
...
```{r}
renderPlotly({
  p2 <- df %>% filter(fighter_dob > input$dob_input[1], fighter_dob < input$dob_input[2]) %>% ggplot(aes(x=fighter_reach_cm, color = over_under_500)) + geom_density() + theme_dark() + ggtitle("Comparison of Fighter Reach (cm)\nby Record") + labs(caption = "Based on 1994-2023 UFC Fighter Data") + theme(plot.caption = element_text(hjust=1)) + theme(plot.title = element_text(hjust=0.5)) + theme(plot.title = element_text(size=10))
