...
df %>% filter(df['artist(s)_name'] == 'The Weeknd') %>% ggplot() + geom_point(mapping = aes(x=in_apple_playlists, y=in_spotify_playlists), color = 'Green') + ggtitle("Comparison of Song Appearances in Spotify vs Apple Playlists for The Weeknd") + theme_dark()
...
df %>% filter(!is.na(key)) %>% group_by(key) %>% summarise(avg_bpm = mean(bpm)) %>% ggplot() + geom_col(mapping = aes(x = key, y = avg_bpm), fill = 'Green', color = 'Black') + theme_dark() + ggtitle("Average Beats per Minute of Spotify Songs Based on their Key")
